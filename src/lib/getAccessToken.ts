...
export default async function getAccessToken(): Promise<string> {
  ...
  const hasRequiredScopes = requiredScopes.every(scope => json.scope.includes(scope));

  if (!hasRequiredScopes) {
    throw new Error(
      `Scope is not correct: ${JSON.stringify(json, null, 2)}`
    )
  }

  if (json.error === 'invalid_grant') {
    throw new Error(
      `Token has been expired or revoked: ${JSON.stringify(json, null, 2)}`
    )
  }
